...
for SPEC_BENCHMARK in $SPEC_BENCHMARKS; do
	...
	qsub -V -N $JOB_NAME -l h_rt=$MAX_TIME_PER_RUN,h_data=$MAX_MEM_PER_RUN -M $MAILING_LIST ./run_gem5_x86_speccpu2006_benchmark.sh $SPEC_BENCHMARK $GEM5_CONFIG_SUBSCRIPT_SPEC $SIM_OUTPUT_DIR
done
...
for BENCHMARK in $BENCHMARKS; do
	...
	qsub -V -N $JOB_NAME -l h_rt=$MAX_TIME_PER_RUN,h_data=$MAX_MEM_PER_RUN -M $MAILING_LIST ./run_gem5_x86.sh $BENCHMARK $SIM_OUTPUT_DIR
done
